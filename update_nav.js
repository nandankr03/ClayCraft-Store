...
const htmlFiles = fs.readdirSync(dir).filter(f => f.endsWith('.html') && !f.includes('dashboard') && !f.includes('payment'));
...
const newNav = `  <nav id="mainNav">
    ...
    <ul id="navLinks" class="nav-main-links">
      ...
      <li class="dropdown logged-in-only" style="display:none;">
        <a href="#" id="userNameDisplay">Hi, User &#9662;</a>
        <div class="dropdown-content">
...
htmlFiles.forEach(file => {
  ...
  // Regex to replace `<nav id="mainNav"> ... </nav>` completely.
  content = content.replace(/<nav\s+id="mainNav">[\s\S]*?<\/nav>/i, newNav);

  // Since some files might have an older nav without id="mainNav" (unlikely), fallback:
